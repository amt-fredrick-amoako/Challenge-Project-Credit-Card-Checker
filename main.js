...
const validateCred = (arr) => {
    ...
    for(let counter = arr.length - 1; counter >= 0; counter--){
        let newVal = arr[counter];
        if((arr.length - 1 - counter) % 2 === 1){
            newVal *= 2
            if(newVal > 9){
                newVal -= 9;
            }
...
const findInvalidCards = (cards) => {
    const invalidCards = [];
    for(let arrCounter = 0; arrCounter < cards.length; arrCounter++){
        let currCred = cards[arrCounter];
...
const idInvalidCardCompanies = (badBatch) => {
    const companies = [];
    for (let invCards = 0; invCards < badBatch.length; invCards++){
        switch(badBatch[invCards][0]){
            case 3:
